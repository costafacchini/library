class Dashboard::Member
  ...
  def load_my_books(user)
    ...
    books.each do |book|
      ...
      my_books << Struct.new(:id, :title, :borrowed_at, :due_date).new(book.id, book.title, borrow.borrowed_at, borrow.due_date)
    end
  ...
  def load_overdue(user, today)
    ...
    books.each do |book|
      overdue << Struct.new(:id, :title, :due_date).new(book.id, book.title, book.borrows.first.due_date)
    end
