...
RSpec.describe Dashboard::Librarian do
  describe '#total' do
  ...
  describe '#total_borrowed' do
    it 'returns the total of books borrowed in the library' do
      member = create(:user, :member, email: 'john@doe.com')
      ...
      subject.load(nil, nil)

      expect(subject.total_borrowed).to be 2
    end
  end
  ...
  describe '#due_today' do
    it 'returns a list of books that due today' do
      ...
      create(:borrow, member:, returned_at: nil)
      create(:borrow, member:, returned_at: Date.new(2024, 5, 8))
      create(:borrow, member:, book: book1, borrowed_at: Date.new(2024, 6, 14), due_date: Date.new(2024, 6, 28),
                      returned_at: nil)

      subject = described_class.new
      subject.load(nil, Date.new(2024, 6, 28))

      expect(subject.due_today.size).to be 1
      expect(subject.due_today[0].id).to eql book1.id
      expect(subject.due_today[0].title).to eql 'Book 1'
  ...
  describe '#members_overdue' do
    ...
    it 'returns a list of members with their books overdue' do
      ...
      create(:borrow, member: member1, book: book1, borrowed_at: Date.new(2024, 6, 13),
                      due_date: Date.new(2024, 6, 27), returned_at: nil)
      create(:borrow, member: member2, book: book2, borrowed_at: Date.new(2024, 6, 13),
                      due_date: Date.new(2024, 6, 27), returned_at: nil)
      create(:borrow, member: member3, book: book3, borrowed_at: Date.new(2024, 6, 13),
                      due_date: Date.new(2024, 6, 27), returned_at: Date.new(2024, 5, 8))
      create(:borrow, member: member1, book: book4, borrowed_at: Date.new(2024, 6, 14),
                      due_date: Date.new(2024, 6, 28), returned_at: nil)
